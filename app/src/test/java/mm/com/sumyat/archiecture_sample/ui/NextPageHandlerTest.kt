...
class NextPageHandlerTest {
    ...
    private lateinit var pageHandler: MoviesViewModel.NextPageHandler

    @Before
    fun init() {
        pageHandler = MoviesViewModel.NextPageHandler(repository)
    }

    private val status: MoviesViewModel.LoadMoreState?
        get() = pageHandler.loadMoreState.value

    @Test
    fun constructor() {
        val initial = status
        assertThat<MoviesViewModel.LoadMoreState>(initial, notNullValue())
    ...
    private fun enqueueResponse(): MutableLiveData<Resource<Boolean>> {
